...
class Display(object):
    ...
    SELECTOR_WIDTH = 0.47
    ROW_OFFSET = 6.0
    TITLES = ['{0} r_e_c_u_r {0}'.format(
        ' '*18), '{0} c_o_n_j_u_r {1}'.format(' '*18, ' '*16), '{0} d_e_t_o_u_r {1}'.format(' '*18, ' '*16)]
    ...
    def _load_browser(self):
        ...
        for index in range(number_of_browser_items):
            ...
            if index >= self.browser_menu.top_menu_index:
                ...
                self.display_text.insert(END, '{:40} {:5} \n'.format(
                    path['name'][0:38], path['slot']))
                number_of_lines_displayed = number_of_lines_displayed + 1

        for index in range(self.MENU_HEIGHT - number_of_browser_items):
            self.display_text.insert(END, '\n')

        self._highlight_this_row(self.browser_menu.selected_list_index -
                                 self.browser_menu.top_menu_index)
    ...
    def _load_settings(self):
        ...
        for index in range(number_of_settings_items):
            ...
            if index >= self.settings_menu.top_menu_index:
                ...
                self.display_text.insert(END, '{:<23} {:<22} \n'.format(
                    setting['name'][0:22], setting['value']))
                line_count = line_count + 1

        for index in range(self.MENU_HEIGHT - number_of_settings_items):
            self.display_text.insert(END, '\n')

        self._highlight_this_row(self.settings_menu.selected_list_index -
                                 self.settings_menu.top_menu_index)
    ...
    def _load_shaders(self):
        ...
        for i in range(min(4, shader['param_number'])):
            display_param = self.format_param_value(
                self.shaders.selected_param_list[self.data.shader_layer][i])
            if display_param == 100:
                display_param == 99
            self.display_text.insert(END, ' {}:{:02d}'.format(i, display_param))
        self.display_text.insert(END, '\n')
        self.display_text.tag_add("COLUMN_NAME", 5.0, 6.0)
        ...
        for index in range(number_of_shader_items):
            ...
            if index >= self.shaders.shaders_menu.top_menu_index:
                ...
                self.display_text.insert(END, '{:<40} {:<5} \n'.format(
                    shader_line['name'][0:30], shader_line['shad_type']))
                line_count = line_count + 1
        for index in range(self.MENU_HEIGHT - number_of_shader_items):
            self.display_text.insert(END, '\n')
        self._highlight_this_row(self.shaders.shaders_menu.selected_list_index -
                                 self.shaders.shaders_menu.top_menu_index)
    ...
    def _load_shader_bank(self):
        ...
        for index, slot in enumerate(shader_bank_data):
            ...
            self.display_text.insert(END, '{:^6} {:<17} {:<5} \n'.format(
                index, name_without_extension[0:17], slot['shad_type']))
            if index % 2:
                self.display_text.tag_add("ZEBRA_STRIPE", self.ROW_OFFSET + index,
                                          self.ROW_OFFSET + self.SELECTOR_WIDTH + index)
        # highlight the slot of the selected player
...
# showing current detour info:
        self.display_text.insert(END, '{:^23} {:^22} \n'.format('SETTING', 'VALUE'))
        self.display_text.insert(END, '{:>23} {:<22} \n'.format(
            "DETOUR_ACTIVE", self.data.detour_active))
        for index, (key, value) in enumerate(self.data.detour_settings.items()):
            if index < 8:
                self.display_text.insert(END, '{:>23} {:<22} \n'.format(key, value))
        detour_banner = self.create_detour_display_banner(
            self.data.detour_settings['detour_size'], self.data.detour_settings['detour_position'], self.data.detour_settings['detour_start'], self.data.detour_settings['detour_end'])
    ...
    def _load_message(self):
        if self.message_handler.current_message[1]:
            ...
            self.display_text.insert(END, '{:8} {:<28} {:>5} \n'.format(
                'CONTROL:', self.data.control_mode, feedback))
    ...
    def _set_colour_from_alpha(self, now_alpha, preview_alpha, next_alpha):
        ...
        self.display_text.tag_configure(
            "CAPTURE_ALPHA", background="black", foreground=capture_colour)
    ...
    def _generate_body_title(self):
        ...
        output = ('=' * (19 - (selected_list_index * 4))) + selected_string + \
